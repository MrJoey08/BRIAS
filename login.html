...
function initTypewriter(elementId) {
  ...
  function tick() {
    ...
    if (!del) {
      ...
      ci++;
      if (ci >= p.length) { setTimeout(function() { del = true; tick(); }, 2800); return; }
      ...
      if (ci < 0) {
        del = false; ci = 0; qi++;
        if (qi >= queue.length) { queue = TW_PHRASES.slice(); shuffleArray(queue); qi = 0; }
